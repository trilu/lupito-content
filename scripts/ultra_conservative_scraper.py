#!/usr/bin/env python3
"""
Ultra-conservative scraper with very long delays
Based on diagnostic success - individual requests work fine
Strategy: 60-120 second delays to completely avoid rate limiting
"""
...
class UltraConservativeScraper:
    def __init__(self, batch_size: int = 10):
        ...
        self.max_delay = 120  # 2 minutes maximum
        ...
        print(f"🐌 ULTRA CONSERVATIVE SCRAPER")
        print(f"Session: {self.session_id}")
        print(f"Batch size: {batch_size}")
        print(f"Ultra-long delays: {self.min_delay}-{self.max_delay} seconds")
        print(f"Expected rate: ~30-60 products/hour")
    
    def get_products(self) -> List[Dict]:
        """Get small batch of products to scrape"""
    ...
    def scrape_with_proven_params(self, url: str) -> Dict:
        """Use stealth mode parameters that worked in diagnostic"""
        
        # Clean URL
        if '?activeVariant=' in url:
            url = url.split('?activeVariant=')[0]
        
        # Use exact parameters from successful diagnostic test
        params = {
    ...
    def run_ultra_conservative_batch(self):
        """Run with ultra-long delays"""
        ...
        print(f"\n🐌 Starting ultra-conservative batch of {len(products)} products")
        print(f"Delays: {self.min_delay/60:.1f}-{self.max_delay/60:.1f} minutes between requests")
        print("-" * 60)
        
        for i, product in enumerate(products, 1):
            # Stop if 3 consecutive errors
            ...
            # Scrape
            result = self.scrape_with_proven_params(product['product_url'])
            ...
            if 'error' in result:
                ...
                if success_indicators:
                    print(f"  ✅ Success: {', '.join(success_indicators)}")
                else:
    ...
    def print_summary(self):
        """Print summary"""
        print("\n" + "=" * 60)
        print("ULTRA CONSERVATIVE SCRAPING COMPLETE")
        print("=" * 60)
...
def main():
    scraper = UltraConservativeScraper(batch_size=5)  # Very small batch
    scraper.run_ultra_conservative_batch()
