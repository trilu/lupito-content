#!/usr/bin/env python3
"""
Fixed Wikipedia Breed Scraper for breeds_details Table
========================================================
Scrapes breed data from Wikipedia with improved parsing logic
"""
...
class WikipediaBreedScraper:
    """Fixed scraper for Wikipedia breed pages"""
    ...
    def scrape_breed(self, breed_name: str, url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Extract data from article content (for missing data like lifespan and weight)
            content_data = self._extract_from_content(soup)
            
            # Merge data (infobox takes precedence)
            breed_data = {
                'breed_slug': self._create_slug(breed_name),
                'display_name': breed_name,
                'raw_html': response.text[:50000],  # Store first 50k chars
                **content_data,  # Content data first
                **infobox_data,  # Infobox overrides content
            }
            
            # Extract comprehensive content sections
            breed_data.update(self._extract_content_sections(soup))
    ...
    def _extract_infobox(self, soup: BeautifulSoup) -> Dict[str, Any]:
        """Extract data from Wikipedia infobox with fixed parsing"""
        data = {}
    ...
    def _extract_from_content(self, soup: BeautifulSoup) -> Dict[str, Any]:
        """Extract missing data from article content"""
        data = {}
        ...
        # Look for weight in paragraphs if not found in infobox
        paragraphs = content.find_all('p', limit=10)
        for p in paragraphs:
            text = p.get_text()
            
            # Look for weight patterns
            if 'weigh' in text.lower() or 'weight' in text.lower():
                weights = self._parse_measurements(text, 'weight')
                if weights and 'weight_kg_min' not in data:
                    data.update(weights)
            
            # Look for lifespan patterns
            if ('life expectancy' in text.lower() or 'life span' in text.lower() or 
                'lifespan' in text.lower() or 'average life' in text.lower()):
                lifespan = self._parse_lifespan(text)
                if lifespan and 'lifespan_years_min' not in data:
                    data.update(lifespan)
        
        return data
    
    def _parse_measurements(self, text: str, measure_type: str) -> Dict[str, Any]:
        """Parse height/weight measurements with improved logic"""
        data = {}
        ...
        if measure_type == 'height':
            # Look for range patterns with various dash types
            ...
            # Look for range patterns with various dash types
            kg_pattern = r'(\d+(?:\.\d+)?)[\u2013\u2014\-–—](\d+(?:\.\d+)?)\s*kg'
            lb_pattern = r'(\d+(?:\.\d+)?)[\u2013\u2014\-–—](\d+(?:\.\d+)?)\s*(?:lb|pound)'
            ...
            if kg_matches:
                ...
                # Try to find individual weight values
                kg_individual = re.findall(r'(\d+(?:\.\d+)?)\s*kg', text_lower)
                lb_individual = re.findall(r'(\d+(?:\.\d+)?)\s*(?:lb|pound)', text_lower)
                
                if kg_individual and len(kg_individual) >= 2:
                    values = [float(x) for x in kg_individual]
                    data['weight_kg_min'] = min(values)
                    data['weight_kg_max'] = max(values)
                elif lb_individual and len(lb_individual) >= 2:
                    values = [float(x) * 0.453592 for x in lb_individual]
    ...
    def _parse_lifespan(self, text: str) -> Dict[str, Any]:
        """Parse lifespan from text with improved patterns"""
        data = {}
        
        # Try various patterns
        patterns = [
            r'(\d+)[\u2013\u2014\-–—](\d+)\s*year',  # Range pattern
            r'(\d+(?:\.\d+)?)\s*to\s*(\d+(?:\.\d+)?)\s*year',  # "X to Y years"
            r'average[^.]*?(\d+(?:\.\d+)?)\s*year',  # "average X years"
            r'expectancy[^.]*?(\d+(?:\.\d+)?)\s*year',  # "expectancy X years"
            r'(\d+(?:\.\d+)?)\s*year',  # Single value
        ]
        
        text_lower = text.lower()
        
        for pattern in patterns[:2]:  # Try range patterns first
            matches = re.findall(pattern, text_lower)
            if matches:
                if isinstance(matches[0], tuple):
                    data['lifespan_years_min'] = int(float(matches[0][0]))
                    data['lifespan_years_max'] = int(float(matches[0][1]))
                else:
                    avg_years = int(float(matches[0]))
    ...
    def _extract_content_sections(self, soup: BeautifulSoup) -> Dict[str, Any]:
        ...
        for elem in content.find_all(['h2', 'h3', 'p']):
            ...
            elif elem.name == 'p' and len(current_text) < 5:
    ...
    def _map_to_controlled_vocab(self, breed_data: Dict[str, Any]) -> Dict[str, Any]:
        """Map extracted data to controlled vocabularies with fixed size thresholds"""
        
        # Determine size based on weight (fixed thresholds)
        # IMPORTANT: Always set size based on weight, or NULL if no weight
        if breed_data.get('weight_kg_max'):
            ...
            # Validation: Log if size seems incorrect for breed
            if breed_data.get('display_name'):
                expected_size = self._validate_size_calculation(weight)
                if expected_size != breed_data['size']:
                    logger.warning(f"Size validation mismatch for {breed_data['display_name']}: "
                                 f"weight={weight}kg suggests {expected_size}, but calculated {breed_data['size']}")
        else:
            # If no weight data, explicitly set size to None (not keeping old value)
            breed_data['size'] = None
            logger.info(f"No weight data for {breed_data.get('display_name', 'unknown')} - setting size to NULL")
        
        # Infer traits from temperament text
        temp_text = breed_data.get('temperament_text', '').lower()
        
        # Energy level
        if any(word in temp_text for word in ['high energy', 'very active', 'athletic', 'energetic', 'working dog']):
            breed_data['energy'] = 'high'
        elif any(word in temp_text for word in ['moderate energy', 'moderately active']):
            breed_data['energy'] = 'moderate'
        elif any(word in temp_text for word in ['calm', 'laid-back', 'low energy', 'lazy', 'couch']):
            breed_data['energy'] = 'low'
        else:
            breed_data['energy'] = 'moderate'  # Default
        
        # Trainability
        if any(word in temp_text for word in ['intelligent', 'easy to train', 'trainable', 'obedient', 'eager to please']):
            breed_data['trainability'] = 'easy'
        elif any(word in temp_text for word in ['stubborn', 'independent', 'difficult to train', 'strong-willed']):
        ...
        # Bark level
        if any(word in temp_text for word in ['vocal', 'barker', 'noisy', 'loud', 'bark']):
        ...
        if any(word in temp_text for word in ['friendly', 'sociable', 'gentle', 'good with', 'loving']):
            breed_data['friendliness_to_humans'] = 4
            breed_data['friendliness_to_dogs'] = 3
        elif any(word in temp_text for word in ['aloof', 'reserved', 'independent', 'wary']):
            breed_data['friendliness_to_humans'] = 2
            breed_data['friendliness_to_dogs'] = 2
        else:
            breed_data['friendliness_to_humans'] = 3
            breed_data['friendliness_to_dogs'] = 3
    ...
    def _validate_size_calculation(self, weight: float) -> str:
        """Validate size calculation based on weight"""
        if weight < 4:
            return 'tiny'
        elif weight < 10:
            return 'small'
    ...
    def process_breeds_from_file(self, filename: str = 'wikipedia_urls.txt', limit: int = None):
        ...
        for i, line in enumerate(lines):
            ...
            # Check if already processed (optional - remove for re-scraping)
            breed_slug = self._create_slug(breed_name)
            # existing = self.supabase.table('breeds_details').select('id').eq('breed_slug', breed_slug).execute()
            # if existing.data:
    ...
    def _generate_report(self):
        ...
        with open(report_file, 'w') as f:
            ...
            if self.stats['errors']:
                f.write("\nERRORS:\n")
                for error in self.stats['errors'][:50]:
                    f.write(f"  - {error}\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Fixed Wikipedia Breed Scraper')
