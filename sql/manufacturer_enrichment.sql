-- Manufacturer Enrichment SQL Scripts
-- For enriching foods_published_preview with manufacturer data
...
SET 
    ...
    ingredients_tokens = COALESCE(p.ingredients_tokens, m.harvested_ingredients),
    price_per_kg_eur = COALESCE(p.price_per_kg_eur, m.harvested_price),
    sources = p.sources || jsonb_build_object(
...
SELECT 
    ...
    CASE 
        ...
        WHEN price_per_kg_eur < 0.5 OR price_per_kg_eur > 50 THEN 'PRICE_OUTLIER'
        ELSE 'OK'
...
AND (
    ...
    price_per_kg_eur < 0.5 OR price_per_kg_eur > 50
);
...
WITH brand_quality AS (
    SELECT 
        ...
        -- Price gate (≥70%)
        ROUND(AVG(CASE WHEN price_per_kg_eur IS NOT NULL THEN 1 ELSE 0 END) * 100, 1) as price_pct
    FROM foods_published_preview
    WHERE brand_slug IN (
        SELECT DISTINCT brand_slug 
        FROM manufacturer_harvest_staging
...
WITH qualifying_brands AS (
    ...
    HAVING 
        ...
        AND AVG(CASE WHEN price_per_kg_eur IS NOT NULL THEN 1 ELSE 0 END) >= 0.70
)
SELECT 
    ...
    '  price_per_kg_eur = preview.price_per_kg_eur,' || E'\n' ||
...
after_stats AS (
    SELECT 
        ...
        AVG(CASE WHEN price_per_kg_eur IS NOT NULL THEN 1 ELSE 0 END) * 100 as price_after
    FROM foods_published_preview
    WHERE brand_slug IN (SELECT DISTINCT brand_slug FROM manufacturer_harvest_staging)
    GROUP BY brand_slug
)
