-- ============================================
-- FIX BREEDS_UNIFIED_API VIEW TO AGGREGATE FROM BOTH TABLES (V2)
...
-- Drop the existing view and dependent views
...
SELECT
    ...
    -- Weight: Combine data from all sources
    ...
    -- Height: Combine data from all sources
    ...
    -- ===== BEHAVIORAL TRAITS =====
    -- Keep fields from their respective tables without mixing types
    bp.energy,
    bp.trainability,
    bp.coat_length,
    bp.shedding,
    ...
    -- Additional traits from comprehensive content (different names to avoid conflicts)
    bcc.coat_texture,
    bcc.coat_length AS coat_length_text,  -- Different name since it's a different type
    bcc.shedding AS shedding_text,  -- Different name since it's a different type
    bcc.barking_tendency,
    ...
    ROUND(
        (
            ...
            CASE WHEN bp.coat_length IS NOT NULL THEN 5 ELSE 0 END +
            ...
            CASE WHEN b.nutrition IS NOT NULL THEN 5 ELSE 0 END +
...
LEFT JOIN breeds b
    ON bp.breed_slug = b.breed_slug  -- Using the new breed_slug column
WHERE
...
BEGIN
    ...
    RAISE NOTICE 'Breeds with height data: % (%.1f%%)', with_height, (with_height::NUMERIC / NULLIF(total_breeds, 0) * 100);
    RAISE NOTICE 'Breeds with weight data: % (%.1f%%)', with_weight, (with_weight::NUMERIC / NULLIF(total_breeds, 0) * 100);
    RAISE NOTICE 'Breeds with lifespan data: % (%.1f%%)', with_lifespan, (with_lifespan::NUMERIC / NULLIF(total_breeds, 0) * 100);
